        ...
        It "should Update site scan configuration" {
            ...
            Should -Invoke _callAPI -ParameterFilter {
                $Request.OperationName -eq "UpdateSiteScanConfigurations" `
                    -and $Request.Query -eq 'mutation UpdateSiteScanConfigurations($input:''UpdateSiteScanConfigurationsInput!'') { update_site_scan_configurations(input:''$input'') { site { id name parent_id scope { included_urls excluded_urls } scan_configurations { id } application_logins { id label username } ephemeral email_recipients { id email } } } }' `
