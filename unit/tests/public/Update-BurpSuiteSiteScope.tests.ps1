        ...
        It "should update site scop" {
            ...
            Should -Invoke _callAPI -ParameterFilter {
                $Request.OperationName -eq "UpdateSiteScope" `
                    -and $Request.Query -eq 'mutation UpdateSiteScope($input:''UpdateSiteScopeInput!'') { update_site_scope(input:''$input'') { site { id name parent_id scope { included_urls excluded_urls } scan_configurations { id name } application_logins { id label username } ephemeral email_recipients { id email } } } }' `
